...
@@ -73,10 +75,42 @@ function ChallengeUpdater:init(is_official, phases_data, utilities, score_limit,
 	self.player_death_time_penalty = ArtifactManager:get_modifier_value("TimerDeathPenalty") or 0
 
 	cat_print("Challenge", "ChallengeUpdater init() - Time Penalty Artifact: " .. tostring(self.player_death_time_penalty))
+	kmf.challenge_updater = self
+end
+
+kmf.practice_challenge_wave = function(wave, add_score)
+	if kmf.challenge_updater then
+		local cu = kmf.challenge_updater
+		local new_score = add_score
+		
+		if cu.score_manager then
+			cu.score_manager.score_needs_update = true
+			cu.score_manager.score_buffer = cu.score_manager.score_buffer + add_score
+			new_score = cu.score_manager.current_score + add_score
+			cu.score_manager.current_score = new_score
+		end
+		cu.score = new_score
+
+		cu:check_magick_unlocks(new_score)
+	
+		cu.this_phase_dead_units = nil
+		cu.tagged_units_this_phase = nil
+		cu.current_phase = wave
+		cu.this_phase_max_health = cu.phase_healths[cu.current_phase]
+		cu.last_phase_progress_factor = 0
+		cu.update_progress_timer = 0
+		cu.all_units_are_dead = false
+		cu.current_phase_should_have_progress_bar = cu:phase_should_have_progressbar(wave)
+	
+		cu:invoke_callback("set_phase_progress_enabled", false)
+		cu:sync_phase_data()
+	end
 end
...
@@ -238,6 +272,10 @@ function ChallengeUpdater:handle_player_died(killed_player)
 end
...
@@ -330,6 +368,10 @@ function ChallengeUpdater:handle_character_killed_character(killed_character, ki
 end
...
@@ -353,6 +395,10 @@ function ChallengeUpdater:on_challenge_unit_spawned(u)
 end
...
@@ -369,6 +415,10 @@ function ChallengeUpdater:force_phase(phase)
 end
...
@@ -396,6 +446,10 @@ function ChallengeUpdater:phase_should_have_progressbar(phase_index)
 end
...
@@ -432,12 +486,20 @@ function ChallengeUpdater:phase_has_victory_cond(phase, cond)
 end
...
@@ -448,6 +510,10 @@ function ChallengeUpdater:get_phase_name(phase_index)
 end
...
@@ -501,6 +567,10 @@ function ChallengeUpdater:start()
 end
...
@@ -587,6 +657,9 @@ function ChallengeUpdater:update_action_collections(action_collections, internal
 end
...
@@ -599,6 +672,9 @@ function ChallengeUpdater:update_phases(dt)
...
@@ -719,6 +795,10 @@ function ChallengeUpdater:try_remove_tagged_unit(u)
 end
...
@@ -853,6 +933,10 @@ function ChallengeUpdater:resume()
 end
...
@@ -861,6 +945,10 @@ function ChallengeUpdater:pause_timer()
 end
...
@@ -877,6 +965,10 @@ function ChallengeUpdater:is_elapsed_time_paused()
 end
...
+
 	...
 	self.paused = false
@@ -916,6 +1008,10 @@ function ChallengeUpdater:restart(phases_data)
 end
